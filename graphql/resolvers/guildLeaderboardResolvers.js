const leaderboardResolvers = {
  Query: {
    // Leaderboard: Top item givers
    // topItemGivers: async (_, { worldId, limit }, { prisma }) => {
    //   const givingActions = await prisma.giving_actions.findMany({
    //     where: { world_id: worldId },
    //     select: { player_id: true },
    //   });
    //   const playerCounts = givingActions.reduce((acc, action) => {
    //     acc[action.player_id] = (acc[action.player_id] || 0) + 1;
    //     return acc;
    //   }, {});
    ...
    //     .slice(0, limit);
    //   return Promise.all(
    //     sortedPlayers.map(async ([playerId, count], index) => {
